...
function createCsvResponse(csv: string) {
...
export async function GET() {
  ...
  if (!isAdmin && galleryIds.length === 0) {
    return createCsvResponse(createCsv([header]));
  }
  ...
  return createCsvResponse(createCsv(csvRows));
}